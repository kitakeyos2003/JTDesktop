...
public class Sprite extends Layer {
     ...
     * o is an Image, otherwise they are ignored
     */
    ...
    private synchronized boolean collidesWithPixelLevel(Object o,
        ...
        while (another) {
            ...
            if (t.collX >= t.getWidth() || t.collX + t.collWidth <= 0
                    || t.collY >= t.getHeight() || t.collY + t.collHeight <= 0) // collision rectangle outside frame bounds
            {
                return false;
            }
                    ...
                    for (int row = minR; row <= maxR; row++) {
                        for (int col = minC; col <= maxC; col++) {
                            ...
                            if (cell == 0) {
                                ...
                                for (int y = minY; y <= maxY; y++, c += rW - (maxX - minX + 1)) {
